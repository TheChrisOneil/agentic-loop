...
TXT
    ...
    case "$OUT" in
      *"0 failed"*)
        ...
        CHAIN_REG=$(mktemp "${TMPDIR:-/tmp}/acceptances.XXXXXX")   # explicit template: GNU rejects -t name
        [ -n "$CHAIN_REG" ] || { echo "mktemp gave no scratch register; refusing to use the real one"; exit 1; }
