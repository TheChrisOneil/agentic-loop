...
Generated from `design/loop.design`. 9 steps, 4 gates, one unit of work:
**One credit limit increase request for one customer account**.
...
2. **The judgment step never writes a number a later step checks.** It interprets. It does not
   compute.
3. **The proof is written by code** and checksummed. SHA-256 written with the record and re-checked before the request is routed to the approver
