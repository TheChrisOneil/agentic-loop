#!/usr/bin/env bash
# STEP 5 — assess-sector-fit
# TYPE:  thinking   (the one question a rule cannot answer)
# ACTOR: model
# SCOPE: unit
#
# From the design:
#   Decide whether the sector and trading pattern justify an increase this large for a customer this size
#
...
The question: Decide whether the sector and trading pattern justify an increase this large for a customer this size
...
case "$JUDGE_MODE" in
  stub)
    ...
    [ -n "$LINE" ] || { step_say "5" "assess-sector-fit" "thinking" "no recorded answer for $UNIT"; exit 1; }
  ...
  claude)
    command -v claude >/dev/null || { echo "  5 assess-sector-fit: claude CLI not installed"; exit 1; }
...
ledger "$UNIT" "assess-sector-fit" "$CALL" "mode=$JUDGE_MODE"
step_say "5" "assess-sector-fit" "thinking" "$CALL"
