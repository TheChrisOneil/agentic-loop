# Inbound support messages grouped into cases, roughly 900 a week

Generated from `design/loop.design`. 9 steps, 3 gates, one unit of work:
**One case, being the set of messages about a single problem from one customer**.
...
2. **The judgment step never writes a number a later step checks.** It interprets. It does not
   compute.
3. **The proof is written by code** and checksummed. SHA-256 recorded at write time and re-checked before routing
