...
# ---- batch phase --------------------------------------------------------
for s in 1-ingest 2-cluster 3-select ; do ./steps/$s.sh "-" || { echo "batch step $s failed"; exit 1; }; done
...
while IFS=$'\t' read -r UNIT LABEL VALUE <&3; do
  ...
  for s in 4-gate-intake 5-triage 6-coverage 7-gate-routing 8-prove 9-deliver ; do
