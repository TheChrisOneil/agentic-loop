...
# ---- batch phase --------------------------------------------------------
for s in 1-fetch 2-group 3-select ; do ./steps/$s.sh "-" || { echo "batch step $s failed"; exit 1; }; done
...
while IFS=$'\t' read -r UNIT LABEL VALUE <&3; do
  ...
  for s in 4-gate-scope 5-assess 6-crosscheck 7-gate-coverage 8-prove 9-deliver ; do
