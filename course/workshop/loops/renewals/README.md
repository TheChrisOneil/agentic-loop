# Contract renewals arriving with changed terms, about 40 a quarter

Generated from `design/loop.design`. 9 steps, 3 gates, one unit of work:
**One renewal, with its changed clauses grouped by the obligation they affect**.
...
2. **The judgment step never writes a number a later step checks.** It interprets. It does not
   compute.
3. **The proof is written by code** and checksummed. SHA-256 recorded at write time and re-checked before the pack is sent
