...
#
# From the design:
#   Write a review pack assigned to the named approver
#
...
#   Stop the review pack and have the contracts owner establish who edited the proof and when
#
...
refuse() {
  { echo "# REFUSED — $UNIT"
    ...
    echo
    echo "**Next human action:** Stop the review pack and have the contracts owner establish who edited the proof and when"
    echo
    echo "Assigned to: $APPROVER"
  } > "$OUTBOX/$UNIT.REFUSED.md"
  ledger "$UNIT" "deliver" refused "gate 9"
  step_say "9" "deliver" "gate" "REFUSED — Stop the review pack and have the contracts owner establish who edited the proof and when"
  exit 1
}
