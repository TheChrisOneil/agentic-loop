...
#
#   Refund requests over 500 dollars, roughly 120 a week across three storefronts
#
...
# ---- batch phase --------------------------------------------------------
for s in 1-fetch 2-enrich 3-select ; do ./steps/$s.sh "-" || { echo "batch step $s failed"; exit 1; }; done
...
while IFS=$'\t' read -r UNIT LABEL VALUE <&3; do
  ...
  for s in 4-gate-intake 5-judge 6-recheck 7-gate-amount 8-prove 9-deliver ; do
